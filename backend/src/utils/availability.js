...
export const buildTimeSlots = async ({ settings, date, staffId, durationMinutes = 15 }) => {
  ...
  const activeStaff = staffId ? await Staff.find({ _id: staffId, isActive: true }).select('_id name') : await Staff.find({ isActive: true }).select('_id name');
  ...
  for (let cursor = open; cursor + durationMinutes <= close; cursor += interval) {
    ...
    const slotEnd = cursor + Number(durationMinutes || settings.defaultServiceDurationMinutes || 15);
...
export const findAvailableStaffForSlot = async ({ date, time, durationMinutes = 15 }) => {
  ...
  const activeStaff = await Staff.find({ isActive: true }).select('_id name');
  const start = timeToMinutes(time);
  const end = start + Number(durationMinutes || 15);
