 ...
 * Features: Required field validation, phone/email validation, business-hours enforcement, time-slot lookup, past-time prevention, any-worker assignment, and duplicate worker/date/time prevention.
...
import { sendAppointmentEmails, sendAppointmentConfirmedEmail } from '../utils/email.js';
...
export const getAvailableSlots = asyncHandler(async (req, res) => {
  ...
  const service = serviceId && isValidObjectId(serviceId) ? await Service.findById(serviceId) : null;
  const durationMinutes = service?.durationMinutes || settings.defaultServiceDurationMinutes || 15;
  const slots = await buildTimeSlots({ settings, date, staffId: isValidObjectId(staffId) ? staffId : '', durationMinutes });
...
export const createAppointment = asyncHandler(async (req, res) => {
  ...
  if (!isValidObjectId(req.body.service)) return res.status(400).json({ message: 'Please select a valid service.' });
  ...
  let staff = null;
  const requestedStaffId = isValidObjectId(req.body.staff) ? req.body.staff : '';

  if (requestedStaffId) {
    staff = await Staff.findById(requestedStaffId);
  ...
  sendAppointmentEmails({ appointment, settings }).catch((error) => {
...
export const updateAppointmentStatus = asyncHandler(async (req, res) => {
  ...
  const oldStatus = previousAppointment.status;
  previousAppointment.status = req.body.status;
  const appointment = await previousAppointment.save();

  const becameConfirmed = oldStatus !== 'Confirmed' && appointment.status === 'Confirmed';

  if (becameConfirmed) {
    const settings = await getOrCreateSettings();

    sendAppointmentConfirmedEmail({ appointment, settings }).catch((error) => {
      console.error('[appointments] Confirmation email failed:', error.message);
    });
  }
