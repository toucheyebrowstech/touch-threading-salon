 ...
 * Features: Loading state, closed/unavailable message, selectable slot buttons, unavailable booked/past states, and selected styling.
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
      {slots.map((slot) => {
        ...
        const isSelected = value === normalized.time;
        const isUnavailable = !normalized.available;
        ...
        return (
          <button
            ...
            onClick={() => onChange(normalized.time)}
          ...
          >
            <span className="block">{formatSlotTime(normalized.time)}</span>

            {isUnavailable && (
              <span className="mt-1 block text-[10px] uppercase tracking-wide">
                {normalized.reason || 'Unavailable'}
              </span>
            )}
          </button>
        );
