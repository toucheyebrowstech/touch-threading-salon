 ...
 * Features: Customer fields, service/staff/date selection, live time slots, disabled past/booked times, validation feedback, booking submit, and confirmation message.
  ...
  if (Array.isArray(result)) {
    return result.map((slot) => {
      if (typeof slot === 'string') {
        return { time: slot, available: true, reason: '' };
      }
...
export default function AppointmentForm({ compact = false, services = fallbackServices, staff = fallbackStaff }) {
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      staff: isMongoId(staffParam) ? staffParam : isMongoId(prev.staff) ? prev.staff : ''
    }));
    ...
    async function loadSlots() {
      ...
      try {
        ...
        const result = await salonApi.getAvailableSlots({
          date: form.date,
          staffId: isMongoId(form.staff) ? form.staff : '',
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...form,
        staff: isMongoId(form.staff) ? form.staff : ''
      };

      const result = await salonApi.createAppointment(payload);
      setToast({ type: 'success', message: result.message || 'Appointment request received. We will confirm it soon.' });
      setForm({ ...initialForm, service: form.service, staff: '', date: formatDateForInput() });
    } catch (err) {
        ...
        <label>
          ...
          <select className="salon-input" value={form.staff} onChange={(e) => update('staff', e.target.value)}>
            ...
            {activeStaff.map((worker) => {
              const workerId = isMongoId(worker._id) ? worker._id : '';
              if (!workerId) return null;

              return (
                <option key={worker._id} value={workerId}>{worker.name} - {worker.role}</option>
              );
            })}
          </select>
        </label>
